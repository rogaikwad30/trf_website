                    ...
                    <td>Team-Members: </td><td><input type="text" name="team[]"  value="<?php  echo $str2;?>"  class="form-control name_list" required></td>
									...
									<td><?php $qry2="SELECT * FROM `tags` ";
											...
											while($row=mysqli_fetch_assoc($run1))
											{
												...
												<input type="checkbox" id="<?php  echo $row['name']; ?>" name="name[]"  value= "<?php  echo $row['name'];?>"  class="form-control name_list" <?php foreach($tg as $val){ $qrry="SELECT * FROM `tags` WHERE `id`='$val'";
												$runn2=mysqli_query($con,$qrry); $row1=mysqli_fetch_assoc($runn2);if(strcmp($row['name'],$row1['name'])==0){echo "checked";}}?>  ><label for= "name"><?php  echo $row['name'];?></label>
